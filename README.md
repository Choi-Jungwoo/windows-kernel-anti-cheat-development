...
目前已编写 [Windows 内核驱动入门详细大纲](src/windows-kernel-driver-intro-outline.md)，章节正文尚待展开。下文介绍的是课程规划，实际内容可以查看 [教程目录](src/SUMMARY.md)。

[学习速查表](src/cheat-sheet.md) 用于复习章节要点、查找术语与代码标识的含义。目前文件保持为空，后续随章节正文编写逐步补充。
...
教程以 Windows 10 22H2 x64 为环境基准，虚拟机可选 Hyper-V 或 VMware。第 0 章会分别安排两条搭建路线，讲清宿主与目标机的分工、工具版本、安装配置、调试连接和快照恢复，并给出完成检查。
